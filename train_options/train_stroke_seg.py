...
from models_stroke_seg import DiT_models
import sys
...
def main(args):
    ...
    if rank == 0:
        ...
        logger.info(f"character_path : {args.character_path}")
        logger.info(f"stroke_path : {args.stroke_path}")
    ...
    character_dir=args.character_path
    stroke_dir=args.stroke_path
    dataset=CharacterStrokePairDataset(args.csv_path,
                 args.character_path,
                 args.stroke_path,
                 character_transform=transform,
                 stroke_transform=transform)
    print(character_dir,stroke_dir)
    ...
    logger.info(f"Dataset contains {len(dataset):,} train images ({args.character_path})")
    logger.info(f"Dataset contains {len(dataset):,} gt images ({args.stroke_path})")
    ...
    for epoch in range(args.epochs):
        ...
        for batch in loader:
            ...
            # 归一化时可能用到
            # stroke_nums=batch['stroke_nums'].to(device)


            # x = x.to(device)
            # y = y.to(device)
            source_img=characters[:visual_num].cpu().clone()
            ...
            t = torch.randint(0, diffusion.num_timesteps, (strokes.shape[0],), device=device)     # 改成100，
            ...
            model_kwargs = dict(y=characters,stroke_order=stroke_orders,unicode=unicodes)
            # loss_dict = diffusion.training_losses(model, y, t, model_kwargs)
            
            loss_dict = diffusion.training_losses(model, characters_enc, t, model_kwargs,clean_target=strokes)
            # loss_dict = diffusion.training_losses(model, strokes, t, model_kwargs,clean_target=strokes)
    ...
    if rank==0:
        try:
            ...
            for sample in diffusion.p_sample_loop(
                model.forward, z.shape, z, clip_denoised=False, model_kwargs=eval_model_kwargs, progress=True, device=device,predict_xstart=args.predict_xstart
            ):
            ...
            save_image(sample, os.path.join(experiment_dir,img_name), nrow=len(sample_idx), normalize=True, value_range=(-1, 1))
            save_gt_img(transform,experiment_dir,gt_prefix,gt_sample_idx)
            save_train_img(transform,experiment_dir,train_prefix,sample_idx)
        except Exception as e:
            logger.info(f"训练后采样失败，失败原因: {e}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--character-path", type=str, required=True)
    parser.add_argument("--stroke-path", type=str, required=True)
