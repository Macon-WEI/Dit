...
class ImageEncoder(nn.Module):
    def __init__(self,img_size=256,img_channels=3,vae_size=32,vae_patch_size=2,depth=6,num_heads=8,mlp_ratio=3,hidden_size=512) -> None:
        super().__init__()
...
class DiT(nn.Module):
    ...
    def __init__(
        ...
        self.y_embedder=CondFusion(img_size=con_img_size,img_channels=con_img_channels,vae_size=input_size,vae_patch_size=patch_size,depth=depth,num_heads=num_heads,mlp_ratio=mlp_ratio,hidden_size=hidden_size,stroke_nums=stroke_nums,unicode_nums=unicode_nums)
    ...
    def forward(self, x, t, y,stroke_order,unicode):
        """
        Forward pass of DiT.
        ...
        y = self.y_embedder(y,unicode=unicode,stroke_order=stroke_order)    # (N, D)
        # y=torch.full(t.shape,0,device=x.device)

        stroke_emb=self.stroke_order_embedder(stroke_order)

        # unicode_emb=self.unicode_embedder(unicode)

        # c = t + y                                # (N, D)
        # c = t+y+stroke_emb+unicode_emb                     # (N, D)
        # c = t+y+stroke_emb  
        c=t+stroke_emb
        for block in self.blocks:
            x = block(x, c)                      # (N, T, D)
