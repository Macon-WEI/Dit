...
def create_npz_from_train_folder_3(train_dir, target_size=(256, 256)):
    ...
    data_path = os.path.join(train_dir, "all_samples_1.data")
    npz_path = os.path.join(train_dir, "all_samples_1.npz")
    print("--------------npz_path111111111111111111-------",npz_path)
    # Use memmap to create an array on disk
    samples = np.memmap(data_path, dtype='uint8', mode='w+', shape=(total_samples, *sample_shape))
    ...
    for class_folder in tqdm(class_folders, desc="Writing samples to .npz"):
        image_files = sorted(glob.glob(os.path.join(class_folder, '*.JPEG')) +
                             ...
                             glob.glob(os.path.join(class_folder, '*.png')))
        for image_file in image_files:
            with Image.open(image_file) as sample_pil:
    ...
    samples_array = np.memmap(data_path, dtype='uint8', mode='r', shape=(total_samples, *sample_shape))
    np.savez(npz_path, samples=samples_array)
    del samples_array
